...
순간은 셋, 접촉은 다섯입니다 — H-1, H-2a, H-2b, H-3 설계 검토, H-3 PR 리뷰. 부록의 설정이 도구에 거부되면 H-1 의 후속이 하나 더 생깁니다. 여기에 **단위마다 PR 병합 한 번**이 더해집니다 — 그것은 준비물이 필요 없는 접촉이고, Production Gate 를 사람이 쥐는 값입니다. 각 순간에 에이전트가 무엇을 준비해 넘기는지를 적습니다. 사람이 빈손으로 시작하지 않게 하는 것이 이 절의 목적입니다.
...
| 되돌리기 | 단위 하나가 **브랜치 하나, PR 하나**입니다(`p0-5-compose`). 주 세션이 브랜치에 커밋·push 하고 PR 을 열면 CI(`verify`, `보호 파일 변경 검토`)가 돌고 **사람이 병합**합니다 — `main` 은 브랜치 보호로 직접 push 가 막혀 있습니다. 되돌리기는 그 PR 의 revert. H-1·H-2 의 보호 파일은 사람이 만들어 PR 에 올리고 `harness-change` 라벨을 붙입니다 |
