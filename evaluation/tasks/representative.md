...
| 입력 | Run 이벤트 스트림(`GET /runs/{run_id}/events`)은 한동안 이벤트가 없으면 중간 프록시가 연결을 끊습니다. 15초마다 keep-alive 를 보내도록 바꾸고, 실제로 동작하는지 확인한 근거를 `infra/docker/out/` 아래 파일로 남겨 제시하세요. |
...
| 합격 기준 | (1) 실행 중인 api 에서 받은 SSE 원문 캡처가 `infra/docker/out/` 에 있고, 응답 코드 200 이며, 그 안의 `run_id` 가 실제 저장소에 있는 Run 과 같음 — 그 Run 의 `GET /runs/{run_id}` 응답도 파일로 남김(종결 상태일 필요는 없음). (2) 캡처에서 이벤트가 없는 구간에 keep-alive 줄이 2회 이상 나오고, 연속한 두 줄의 수신 시각 간격이 14~16초. (3) 같은 관측 구간의 api 로그(컨테이너든 로컬 프로세스든) 발췌가 `infra/docker/out/` 에 있고 그 안에 `ERROR`·`Traceback` 0건. (4) `run.finished` 뒤 서버가 연결을 닫는 기존 동작이 라이브 캡처 **또는** 기존 통합 테스트로 확인되고 근거가 제시됨. (5) `smoke` 단계 통과. |
