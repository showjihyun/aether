...
| 합격 기준 | (1) 실행 중인 api 에서 받은 SSE 원문 캡처가 `infra/docker/out/` 에 있고, 그 안에 **같은 `run_id`** 의 `run.status` 와 `run.finished` 가 함께 들어 있음. (2) 그 `run_id` 의 `GET /runs/{run_id}` 응답이 종결 상태이고 그 응답이 파일로 남아 있음. (3) 캡처에서 이벤트가 없는 구간에 keep-alive 줄이 2회 이상 나오고, 연속한 두 줄의 수신 시각 간격이 14~16초. (4) 같은 관측 구간의 api 로그 발췌가 `infra/docker/out/` 에 있고 그 안에 `ERROR`·`Traceback` 0건. (5) `run.finished` 뒤 서버가 연결을 닫는 기존 동작이 캡처에서 유지됨. (6) `smoke` 단계 통과. |
