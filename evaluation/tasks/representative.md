...
| 입력 | `aether-api agents get-version` 의 출력이 이 저장소의 규약과 맞지 않습니다. 무엇이 어긋났는지 찾아 고쳐 주세요. 기존 계층 구조를 따릅니다. |
...
| 실행 가능 | 지금. **평가자가 규약을 어긴 구현을 심어 실행합니다** — 출력에 `Agent` 의 가변 필드나 `Run` 필드를 섞고, 메타데이터를 stdout 에 함께 싣는 식입니다(정답이 git 이력에 남지 않는 형태여야 합니다). 시드의 내용과 시드 직후 상태는 실행 기록에만 남기고 이 문서에는 적지 않습니다. `.eval-blind` 마커를 켜고 실행합니다(blind 조건, [../../scripts/guard-eval-blind.sh](../../scripts/guard-eval-blind.sh)) |
...
| REP-5 | 규약 문서 발견 실패 | `architecture` | 서브커맨드 병합 뒤(시드) |
