...
| 입력 | `aether-api` 로 특정 `Agent Version` 의 정의를 JSON 으로 출력하는 기능이 동작하지 않습니다. 복구해 주세요. 기존 계층 구조를 따릅니다. |
...
| 실행 가능 | 서브커맨드가 병합된 뒤. **평가자가 그것을 지운 시드를 심어 실행합니다**(CLI 어댑터와 조립 지점의 연결을 함께 제거). 시드의 내용과 시드 직후 상태는 실행 기록에만 남기고 이 문서에는 적지 않습니다. `.eval-blind` 마커를 켜고 실행합니다(blind 조건, [../../scripts/guard-eval-blind.sh](../../scripts/guard-eval-blind.sh)) |
