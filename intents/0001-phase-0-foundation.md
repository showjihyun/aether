...
| # | 질문 | 누가 답하는가 | 언제까지 |
| --- | --- | --- | --- |
| 1 | Python 패키지 관리자를 uv / poetry 중 무엇으로 고정하는가. `harness.config` 의 제품 단계 명령이 여기 달려 있습니다 | 사람 | Phase 0 착수 전 |
| 2 | 모노레포 도구를 pnpm workspace 단독으로 갈 것인가, turborepo 를 함께 쓸 것인가 | 사람 | Phase 0 착수 전 |
...
질문 1~3 은 **착수 전에 닫습니다.** `harness.config` 의 제품 단계 명령이 이 답에 직접 의존하므로, 열린 채로 시작하면 에이전트가 추측으로 명령을 적고 그 명령이 게이트가 됩니다.
