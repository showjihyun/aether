...
| 2026-09-16 | **Phase 1 완료 — Agent Runtime 이 돕니다.** P1-9(PR #26 에이전트 부분 + 사람 손 H-6 PR #27 CI bake 캐시 · H-5 PR #28 `harness.config` 의 11번째 제품 단계 `smoke` · H-7 PR #29 성능 기준 150 ms). verify 17단계 로컬 163.4초 / CI 약 3분(R-9), 성능 기준은 사람이 고정(R-10, EI-2). 이로써 `POST /agents` → `POST /agents/{id}/run` → worker 루프 → `GET /runs/{id}` · SSE · trace 가 compose 에서 끝에서 끝까지 동작합니다. intent 0002 완료, 다음은 intent 0003(Phase 2 — Enterprise MCP Gateway) |
| 2026-09-12 | **Phase 1 착수 — P1-2a·P1-2b 완료**(각 PR 병합 시점). P1-2b: `packages/runtime` 이 처음 채워짐 — `RunStatus`·전이표·`RunState`·`LeaseHeld`·`AgentDefinition`·`BUILTIN_TOOL_NAMES`, 포트 `Clock`·`RunStateStore`, PostgreSQL `RunStateStore`(lease 는 DB 시계 조건부 UPDATE, `run_states` UPSERT), fake 둘, 계약 테스트 fake·postgres 공용(PG 만료 경로는 관리자 backdate). P1-3(PR 병합 시점): `ModelGateway` 포트(`ModelRequest`·`ModelResponse`·`ModelDelta`·`ModelError`), `FakeModelGateway`(시나리오 주입), `OpenAICompatibleGateway`(`httpx` 는 이 파일에만, thinking 분리·`think: false`, 스트림 조각 조립, `httpx.MockTransport` 로 네트워크 0), compose `llm` 프로파일(Ollama 0.34.0 digest 고정, 기본 `up`·probe 와 격리), `.env.example` 의 `AETHER_MODEL_*`(`qwen3.8:27b`), README "실제 모델로 돌리기". H-4 후보(`.importlinter`, `error` 설정)가 이 트리에서 11 kept 로 검증됨. P1-4(PR 병합 시점): `ExecuteRunUseCase`(lease·재개·재발행·취소 SELECT·`Observation` 표지·`Task` 마다 span), 포트 `Tool/ToolRegistry`·`EventSink`·`StatusNotifier`·`Tracer`·`RunDeclarationReader`·`ExecuteRun`, 도메인 `Observation`·`RunEvent` 8종, 내장 도구 `clock`·`calculator`(`ast`), `set_status` 에 `started_at`/`finished_at`(계약 테스트 먼저). 리뷰가 결함 4건(집을 때 취소 → `queued→cancelled`, 종결 두 커밋 사이 crash 창, 허용 도구는 정의 기준, `Task` 매 단계)을 반려로 돌려 테스트 먼저 고침. 순서 위반(구현 먼저)을 implementer 가 자진 고지 → mutation 증거 3건 요구, improvement-log `2026-09-13-002`(HE-10, red-check hook 제안). P1-5a(PR 병합 시점): worker 실행 경로 — `HandleRunRequested` 유스케이스(LeaseHeld 면 ack 안 함, 선언 없음은 독약 처리), `RequestedConsumer`(자기 PEL 먼저 → `XAUTOCLAIM` → `XREADGROUP >`, `count=1`), heartbeat 키와 compose healthcheck, runtime 의 `RedisEventSink`(explicit ID, 같은 ID 재-XADD 흡수, MAXLEN·TTL)·`RedisStatusNotifier`·`PostgresRunDeclarationReader`·`SystemClock`·no-op `Tracer`, `FakeModelGateway.echo()`(fake 어댑터로 무한히 `succeeded`), `tests/support/redis.py`. 그룹 7개마다 red 기록을 요구한 첫 단위 — 전부 지킴. P1-5b(PR 병합 시점): Run API 3경로(선언 커밋 → 통지, 취소 멱등, GET 은 투영만), api 안의 `aether:runs:status` 소비자(`seq` 멱등 UPDATE, 커밋 뒤 ack), `PostgresRunDeclarationStore`·`RedisRunNotifier`, sdk `runAgent`·`getRun`·`cancelRun`, e2e(api + worker, `succeeded` 와 gated 도구로 `cancelled`) — **R-2 가 처음 성립**. compose 로 손으로 Run 하나 `succeeded`. P1-6(PR 병합 시점): `GET /runs/{id}/events` SSE(`redis.asyncio` `XREAD BLOCK`, `Last-Event-ID`, 스트림 없음+종결 → `run.finished` 합성, 단절 뒤 Run 계속 — R-3), `aether-api events-schema` → `events.schema.json` → `json2ts` 생성 타입(R-12 둘째 드리프트), sdk `streamRunEvents`(`fetch` 기반 SSE 파서, 테스트 먼저). P1-8(PR 병합 시점): OTel 추적 — runtime `OtelTracer`(provider 주입), worker `OtelTraceContext`(`traceparent` 복원), api `RequestTracing` 포트와 `run.request` span, e2e 에서 api·worker span 이 한 trace·부모–자식이고 `GET /runs/{id}` 의 `trace_id` 가 그 값(mutation 2건), compose `otel-collector`(contrib 0.160.0, debug + file, `infra/docker/out/otel/`) — 실제 Run 의 `trace_id` 를 `spans.jsonl` 에서 확인, spec 0002 개정 6 [편집]. 그룹 전부 테스트 먼저. P1-7(PR 병합 시점): 재시도·백오프·타임아웃 — 기준 시계를 `started_at` + `Clock.now()` 로(재개 뒤 예산 유지), 재시도 가능한 모델 오류 분류, 도구는 예외만 재시도, `ModelRequest.timeout_seconds` 로 잔여 전달, `LeaseKeeper` 포트와 `ThreadedLeaseKeeper`(잃은 단계는 무기록 `LeaseHeld`), spec 0002 개정 5 [편집]. 포트·스레드 어댑터가 테스트보다 먼저 쓰여(자진 고지) 반려 → mutation 증거 4건. **H-4**(PR #18, 사람): `.importlinter` 한 번 — AR-5 `httpx` 금지·SDK ignore 삭제·`error`, AR-7 확장. 사람이 PR #17 병합 전에 블록을 실행해 `error` 가 예측대로 실패한 뒤 병합 후 재실행으로 통과. P1-1(PR 병합 시점): Agent Registry 5경로(`versions/{version}` 포함), `current_version` + `FOR UPDATE` 직렬화·`409`, 평탄 유스케이스 5개와 1:1 테스트, 실제 `.importlinter` 발화 테스트(R-7, `src` 복사 + `PYTHONPATH`), AR-10 구조 테스트, sdk 함수 5개, `docs/api.md` 시작. intent 0002 → spec 0002(리뷰 F-1~F-21, D-1~D-19) → plan 0002(리뷰 F-1~F-19, 11 단위·5 wave, 사람 손 네 접촉) 가 하루에 승인. P1-2a: 테스트 도구(pytest `pythonpath`, mypy `explicit_package_bases`+`mypy_path`, `pytest-socket` loopback 허용, 루트 `conftest.py`), `tests/support/{pg,waiting}.py`, `ast` 기반 `sleep` 금지 구조 테스트(기존 2곳 제거 — 프로덕션 무변경), 마이그레이션 0002(`control.agents.current_version`, `control.runs` 투영 열, lease 열, `data.run_states`). 실행 중 발견: pytest 9 의 `pytest_plugins` 는 rootdir conftest 만 → spec 0002 개정 3 [편집] |
| 2026-09-11 | **P0-9 완료 — Phase 0 완료**(PR #10 병합 시점). 🔒 단위의 절차: implementer(Sonnet)가 1단계(domain·포트 3개·red 테스트)에서 멈춤 → H-3 설계 검토(결정 11건, spec 개정 11·plan 개정 1, 승인 showjihyun) → 2단계 구현(red → green, 유스케이스 2·PostgreSQL 어댑터·HTTP 의존성·CLI `keys create`) → verify 16단계 → H-3 PR 리뷰 → 병합. 1단계의 의도된 red 가 stop 게이트에 막혀 `git stash` 로 보관한 채 검토를 받음(테스트 약화 없이 게이트를 통과하는 유일한 길). intent 0001 완료, 다음은 intent 0002 발급 |
| 2026-09-11 | **P0-7 완료 — AD-2 진입.** PR #3(에이전트 부분: candidate 14건·실측·전사 버그) → PR #4 H-2b(CI 도구 설치·gitleaks·Dependabot, 사람) → PR #5 H-2a(`harness.config` 제품 단계 10개·임계값 80·LINK_DIRS, 사람). verify 16단계 로컬 104.0초 / CI 1분 33초(R-2·R-5·R-11 성립). H-2 순서를 plan 과 반대로 — PR 의 CI 는 PR 의 두 파일로 돌아 config 를 먼저 켜면 도구가 없어 병합이 막힘. H-2b 의 첫 CI 실패 2건(`setup-uv@v10` 이동 태그 없음, gitleaks `GITHUB_TOKEN`) → candidate 015. Dependabot 이 즉시 PR 3건(python 3.14-slim, uv 0.12.13, actions/checkout 7) |
| 2026-09-11 | **P0-5 완료**(PR #2 병합 시점) — compose 3 이미지(digest 고정) + 오프라인 probe + 루트 README. R-1 healthz version = 커밋, R-4 probe exit 0, R-6 이미지에 .env 없음. P0-5 의 red 가 **P0-8 의 버그**를 잡음: `01-roles.sh` 의 DO 블록 안 psql 치환 미적용 — P0-8 테스트는 스크립트를 실행하지 않았음. 후속(LP-2): 스크립트를 실제 실행하는 회귀 테스트 먼저 → `\gexec` 수정. 함께 P0-4 worker 통합 테스트 플레이크(고정 5초 폴링 → `on_ready` 이벤트). `.dockerignore` 의 `.env*` 는 컨텍스트 루트에서만 매칭 → `**/.env*`(spec 개정 8) |
| 2026-09-10 | SDLC 리뷰 — 사슬의 앞(요구→계획)은 성립, 뒤(리뷰·병합·평가·학습)는 문서만 있고 돌지 않았음: PR 0건·`main` 직접 push 32회·`protected` job 미실행·improvement-log 0·평가 실행 0. 적용: `main` 브랜치 보호(PR 필수, `verify`·`보호 파일 변경 검토` 필수, 관리자 포함) + 단위 = 브랜치 + PR + 사람 병합(AGENTS.md Loop), intent `완료` 상태, spec 개정을 [실질]/[편집] 으로 구분(specs/README), 테스트 docstring 에 근거 R/AR/D, 완료의 정의(backlog), P0-5 에 버전 = `git describe`, P0-7 에 그림자 로그 일괄 전환·평가 기준선·Dependabot. 이 변경이 첫 PR |
