...
intent 와 spec 은 짝으로 커밋합니다. 요구사항과 그때의 결정이 한 기록으로 남아야 "왜 이렇게 만들었는가" 를 나중에 복원할 수 있습니다. 승인 전에는 자유롭게 고칩니다. 승인 뒤의 개정은 머리 표에 행으로 남기되 **두 종류를 가릅니다**:

| 구분 | 무엇 | 누가 승인 |
| --- | --- | --- |
| **[실질]** | 요구사항·결정(R·D)이 바뀌거나 새로 생김. 구현이 달라짐 | 사람. 지시 또는 PR 승인이 기록 |
| **[편집]** | 구현에서 드러난 사실을 명세에 맞춰 적음(경로·명령·nullable 같은). 요구사항의 뜻은 그대로 | 주 세션이 리뷰에서. PR 로 사람이 볼 수 있음 |

[편집] 이 요구사항의 뜻을 바꾸고 있다면 그것은 [실질] 이고, 사람에게 물어야 합니다.
